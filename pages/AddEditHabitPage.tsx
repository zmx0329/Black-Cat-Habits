...
const AddEditHabitPage: React.FC = () => {
  ...
  const handleSubmit = () => {
    ...
    if (isEdit && id) {
      updateHabit({ ...formData, id } as Habit);
    } else {
      addHabit({
        ...formData,
          ...
          <div data-purpose="segmented-control">
            ...
            <div className="p-[2px] rounded-[8px] flex items-center h-10 bg-[#7676801F] gap-0">
              <button 
                ...
                className={`w-1/2 flex items-center justify-center space-x-2 rounded-[7px] h-full font-medium transition-all ${
                    formData.type === HabitType.GOOD 
            ...
            <div className="space-y-5">
                  ...
                  {days.map((day, index) => {
                      ...
                      return (
                        <button 
                            ...
                            className={`aspect-square rounded-xl flex items-center justify-center text-xs font-bold shadow-sm transition-all ${
                                isSelected ? 'bg-[#38393D] text-white' : 'bg-white border border-gray-200 text-gray-400 hover:bg-gray-50'
                            }`}
                        >
                            {day}
                        </button>
              ...
              <div data-purpose="daily-times">
                ...
                <div className="bg-[#7676801F] rounded-xl p-1.5 flex items-center justify-between h-14">
                  <button 
                    type="button"
                    onClick={() => setFormData({ ...formData, dailyGoal: Math.max(1, (formData.dailyGoal || 1) - 1) })}
                  ...
                  <div className="flex-1 text-center flex items-center justify-center">
                    <span className="text-xl font-bold text-gray-900 leading-none">{formData.dailyGoal}</span>
                  </div>
                  <button 
                    type="button"
                    onClick={() => setFormData({ ...formData, dailyGoal: (formData.dailyGoal || 1) + 1 })}
