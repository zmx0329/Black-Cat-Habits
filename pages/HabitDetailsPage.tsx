...
const HabitDetailsPage: React.FC = () => {
  ...
  const { habits, logs, addLog, deleteLog } = useApp();
  const habit = habits.find(h => h.id === id);
  const habitLogs = logs.filter(l => l.habitId === id).sort((a, b) => b.timestamp - a.timestamp);
      ...
      for (let i = 5; i >= 0; i--) {
          ...
          const monthCols = colRanges.map((range, colIdx) => {
              ...
              for(let day = range.start; day <= range.end; day++) {
                  ...
                  if (count > 0) {
                      opacity = Math.min(1, Math.max(0.4, count / habit.dailyGoal));
                  } else {
                      // Make some random past data for visual
                      if (checkDate < new Date() && Math.random() > 0.7) opacity = 0.6;
                  }
          ...
          <div className="bg-[#f2f2f7] rounded-[20px] p-5 shadow-sm relative overflow-hidden">
            <div className="mb-4">
              ...
              <div className="bg-white/60 rounded-xl p-3 space-y-1.5 border border-black/5">
                <p className="text-sm text-gray-800"><span className="text-gray-500 font-medium">目标：</span>今日 {habit.dailyGoal} 次，本周 {habit.frequency.length} 次</p>
                <p className="text-sm text-gray-800"><span className="text-gray-500 font-medium">累计打卡：</span>今日 {habit.todayCount} 次，本周 {habit.streak} 次</p>
              </div>
            </div>
