...
const HomePage: React.FC = () => {
  const { habits, addLog, deleteLog, reorderHabits } = useApp();
  ...
  const performCheckIn = (habit: Habit) => {
    if (isSorting) return;
  ...
  const handleEditFromModal = () => {
      if(selectedHabit) {
          navigate(`/edit/${selectedHabit.id}`);
      }
  };
  ...
  return (
    <div className="pb-24 bg-[#F2F2F7] min-h-screen relative">
      {/* Header */}
      <div className="flex justify-between items-center px-6 py-2 text-sm font-semibold select-none bg-[#F2F2F7]">
        <span>9:41</span>
          ...
          {habits.map((habit, index) => (
            ...
            >
              ...
              <div className="mb-1.5 flex flex-wrap items-baseline gap-x-2 pr-4 mt-2">
                ...
                <span className="text-[10px] text-gray-500 font-medium">今日 {habit.todayCount} 次，本周 5 天</span>
              </div>
              <div className="mb-3 text-[10px] text-gray-400">
                目标：今日 {habit.dailyGoal} 次，本周 7 天
              </div>
              <div className="flex items-center gap-1.5 mt-auto pr-6">
                <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-bold flex-shrink-0 ${
                    habit.type === HabitType.GOOD ? 'text-[#15803d] bg-[#dcfce7]' : 'text-[#b91c1c] bg-[#fee2e2]'
                }`}>
                ...
                <div className="flex flex-col items-center text-center">
                    <div className="mb-6 flex flex-col items-center">
                        <div className="rounded-full bg-green-50 p-2 mb-3">
                            <span className="material-symbols-outlined text-6xl text-[#16a34a] symbol-filled">check_circle</span>
                        </div>
